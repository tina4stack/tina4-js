...
(new Get()).add( '/test', function(response, request) {
    let content = Tina4.renderTemplate(`<h1>Hello {{name}}!</h1><form target="root" method="post"><input type="text" name="firstName" value="{{firstName}}"><button>Send</button></form>`, {name: "Andre", firstName: "Andre"});
    return response (content, 200, 'text/html')
});

(new Get()).add( '/test/{id}', function(response, request) {
    console.log (request);
    let content = Tina4.renderTemplate(`<h1>Hello parsing params ok {{id}}!</h1>`, request);
    return response (content, 200, 'text/html')
});
